...
if WORKSPACE_ID is None or SHARED_KEY is None:
    raise Exception(
        "Please add azure sentinel customer_id and shared_key to azure key vault/application settings of web app"
    )

BASIC_AUTH = base64.b64encode(f"{WORKSPACE_ID}:{SHARED_KEY}".encode()).decode("utf-8")
LOG_TYPE = "Log-Type"
HTTPS = "https://"
...
def post(headers, body, auth):
    auth_string = " auth " if auth else " "
    response = POOL.post(URI, data=body, headers=headers)
    if response.status_code >= 200 and response.status_code <= 299:
        logging.debug("accepted %s", auth_string)
    else:
        resp_body = str(response.json())
        resp_headers = json.dumps(headers)
        failure_resp = f"failure{auth_string}response details: {response.status_code}{resp_body}{resp_headers}"
        raise ProcessingException(
            f"ProcessingException for{auth_string}: {failure_resp}"
        )
...
def func():
    ...
    try:
        for auth in auth_headers:
            if "Basic" in auth:
                ...
                if basic_auth_header.split("Basic ")[1] != BASIC_AUTH:
                    logging.error(
                        "UnAuthorized Basic header mismatch %s vs %s",
                        basic_auth_header,
                        BASIC_AUTH,
                    )
        ...
        xms_date = ", ".join(
            [each.strip() for each in request.headers.get("x-ms-date").split(",")]
        ).replace("UTC", "GMT")
        headers = {
        ...
        if decomp_body_length == 0:
            if len(body) == 0:
                logging.error("decompressed: %s vs body: %s", decompressed, body)
        ...
        logging.error("ValueError: %s%s%s", headers, e, decompressed)
        return FAILURE_RESPONSE, 500, APPLICATION_JSON
